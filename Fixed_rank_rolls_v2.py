...
def rank_output(dataAna):
    ...
    for i in dataAna:
        ...
        demand_sub_rolls[i[1]-1].append(i[0])
...
def rank_rolls(list_temp,ID_list_temp):
            ...
            for i, j in zip(groupNo,dataClean):
                for l in range(len(i)):
                    ...
                    if str((i[l][-1])).__contains__('A'):
                        ...
                        dataAna.append([sql_ID, sql_Group_No, sql_Length_NC, str(sql_Length_A)+"-"+str(sql_Length_NC), 0, anaInfo])#i[l][-1].strip('A')
                    elif str((i[l][-1])).__contains__('B'):
                        sql_Length_B = j[l]
                        sql_Length_NC = originDict1[str(i[l]).strip('B')] #原杆件长度
                        dataAna.append([sql_ID,sql_Group_No,sql_Length_NC,0,str(sql_Length_B)+"-"+str(sql_Length_NC),anaInfo])#i[l][-1].strip('B')
    ...
    sql = "INSERT INTO Analysis_table (ID,Group_No,Length_NC,Length_A,Length_B,INFO) VALUES (?,?,?,?,?,?)"
