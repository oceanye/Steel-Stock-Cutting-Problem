...
def gen_data(n):
    ...
    S=0
    for i in range(n):
        R.append([randint(1,20), randint(500,12000)])
    return R
...
def bounds(D):
  ...
  for i in range(n):
    q,w = D[i][0], D[i][1]
    b.append(min(D[i][0],int(round(12000/D[i][1]))))
    if T+q*w <= 12000:
      T,TT = T+q*w,TT + q*w
    else:
      while q:
...
def solve_master(C,b,integer=False):
  ...
  l0=[u[i].DualValue() for i in range(m)]
  l=[0 if integer else u[i].DualValue() for i in range(m)]
  return rc, y, l

def get_new_pattern(l,w):
  s = newSolver('Cutting stock slave', True)
  n = len(l)
  a = [s.IntVar(0,12000,'') for i in range(n)]
  Cost = sum(l[i]*a[i] for i in range(n))
  s.Maximize(Cost)
  s.Add(sum(w[i]*a[i] for i in range(n)) <= 12000)
  rc = s.Solve()
